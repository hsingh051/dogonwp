...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.1.1', __( 'WooCommerce Braintree Gateway', 'woocommerce-gateway-braintree' ), __FILE__, 'init_woocommerce_gateway_braintree', array( 'minimum_wc_version' => '2.2', 'backwards_compatible' => '4.0.0' ) );
...
class WC_Braintree extends SV_WC_Plugin {
	...
	public function includes() {
		...
		// exception class
		require_once( $this->get_plugin_path() . '/includes/class-wc-gateway-braintree-exception.php' );
		...
		if ( $this->is_subscriptions_active() || $this->is_pre_orders_active() ) {
			...
			require_once( $this->get_plugin_path() . '/includes/class-wc-gateway-braintree-addons.php' );
	...
	public function include_template_functions() {
		include_once( $this->get_plugin_path() . '/includes/wc-gateway-braintree-template.php' );
	}
		...
		if ( isset( $settings['enabled'] ) && 'yes' == $settings['enabled'] ) {
			if ( isset( $settings['environment'] ) && 'production' == $settings['environment'] ) {
				...
				if ( 'no' === get_option( 'woocommerce_force_ssl_checkout' ) && ! $this->get_admin_notice_handler()->is_notice_dismissed( 'ssl-recommended-notice' ) ) {
					...
					$this->get_admin_notice_handler()->add_admin_notice( __( 'WooCommerce is not being forced over SSL -- Braintree recommends forcing the checkout over SSL for maximum security. ', self::TEXT_DOMAIN ), 'ssl-recommended-notice' );
				}
			}
		}
	}
