...
class WC_Gateway_Braintree extends WC_Payment_Gateway {
	...
	public function enqueue_js() {
		...
		wp_enqueue_script( 'wc-braintree', wc_braintree()->get_plugin_url() . '/assets/js/frontend/wc-braintree.min.js', array( 'braintree-js' ), WC_Braintree::VERSION, true );
	...
	public function is_available() {
		...
		if ( ! $this->get_merchant_id() || ! $this->get_public_key() || ! $this->get_private_key() || ! $this->get_cse_key() || ! $this->get_environment() ) {
			$is_available = false;
		}
		...
		if ( $this->icon ) {
			...
			foreach ( $this->card_types as $card_type ) {
				...
				if ( is_readable( wc_braintree()->get_plugin_path() . '/assets/images/card-' . strtolower( $card_type ) . '.png' ) ) {
					$icon .= '<img src="' . esc_url( WC_HTTPS::force_https_url( wc_braintree()->get_plugin_url() ) . '/assets/images/card-' . strtolower( $card_type ) . '.png' ) . '" alt="' . esc_attr( strtolower( $card_type ) ) . '" />';
				}
			}
		}
	...
	protected function get_order( $order_id ) {
		...
		$braintree_customer_id = get_user_meta( $order->get_user_id(), '_wc_braintree_customer_id', true );

		// get selected payment token
		$braintree_cc_token = SV_WC_Helper::get_post( 'braintree-cc-token' );
	...
	protected function create_customer( $order ) {
		...
		$customer = array(
			...
			'creditCard' => array(
				'number'          => SV_WC_Helper::get_post( 'number' ),
				'expirationMonth' => SV_WC_Helper::get_post( 'month' ),
				'expirationYear'  => SV_WC_Helper::get_post( 'year' ),
		...
		if ( $this->is_cvv_required() ) {
			$customer['creditCard']['cvv'] = SV_WC_Helper::get_post( 'cvv' );
		}
		...
		if ( $response->success ) {
			...
			if ( is_user_logged_in() || 0 !== $order->get_user_id() )  {
				add_user_meta( $order->get_user_id(), '_wc_braintree_customer_id', $order->braintree_order['customerId'] );
			}
	...
	protected function create_credit_card( $order ) {
		...
		// add CVV
		if ( $this->is_cvv_required() ) {
			$credit_card['cvv'] = SV_WC_Helper::get_post( 'cvv' );
		}
	...
	protected function do_transaction( $order ) {
		...
		// add CVV
		if ( $this->is_cvv_required() && SV_WC_Helper::get_post( 'cvv' ) ) {
			$order->braintree_order['creditCard']['cvv'] = SV_WC_Helper::get_post( 'cvv' );
		}
		...
		if ( $response->success ) {
			...
			update_user_meta( $order->get_user_id(), '_wc_braintree_customer_id', $response->transaction->customerDetails->id );
	...
	protected function mark_order_as_failed( $order, $message = '' ) {
		...
		SV_WC_Helper::wc_add_notice( __( 'An error occurred, please try again or try an alternate form of payment.', WC_Braintree::TEXT_DOMAIN ), 'error' );
